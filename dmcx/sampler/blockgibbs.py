...
class BlockGibbsSampler(abstractsampler.AbstractSampler):
  ...
  def step(self, model, rnd, x, model_param, new_state):
    ...
    def get_new_sample(rnd_new_sample, x, model, model_param):
      ...
      for dim in range(0, len(indices), self.block_size):
        ...
        category_indeces = jnp.vstack([categories_iter] * x.shape[0])
        indices_to_flip = indices[dim:dim + self.block_size]
        category_indeces = jax.lax.dynamic_slice(
            category_indeces, (0, 0),
            (category_indeces.shape[0], indices_to_flip.shape[0]))
        x_expanded = x_expanded.at[:, indices_to_flip].set(category_indeces)
        ...
        new_x_dim = random.categorical(rnd_categorical, loglikelihood, axis=1)
        x = x.at[:, dim].set(new_x_dim)
      return x
