...
class FlaxBertForPreTrainingOutput(ModelOutput):
  ...
  Args: prediction_logits (:obj:`jax_xla.DeviceArray` of shape
  :obj:`(batch_size, sequence_length, config.vocab_size)`): Prediction scores of
  the language modeling head (scores for each vocabulary token before SoftMax).
  seq_relationship_logits (:obj:`jax_xla.DeviceArray` of shape
  :obj:`(batch_size, 2)`): Prediction scores of the next sequence prediction
  (classification) head (scores of True/False continuation before SoftMax).
  hidden_states (:obj:`tuple(jax_xla.DeviceArray)`, `optional`, returned when
  ``output_hidden_states=True`` is passed or when
  ``config.output_hidden_states=True``): Tuple of :obj:`jax_xla.DeviceArray`
  (one for the output of the embeddings + one for the output of each layer) of
  shape :obj:`(batch_size, sequence_length, hidden_size)`. Hidden-states of the
  model at the output of each layer plus the initial embedding outputs.
  attentions (:obj:`tuple(jax_xla.DeviceArray)`, `optional`, returned when
  ``output_attentions=True`` is passed or when
  ``config.output_attentions=True``): Tuple of :obj:`jax_xla.DeviceArray` (one
  for each layer) of shape :obj:`(batch_size, num_heads, sequence_length,
  sequence_length)`. Attentions weights after the attention softmax, used to
...
class FlaxBertEmbeddings(nn.Module):
  ...
  def one_hot_embeddings(self, one_hot_array):
    """one_hot_array: 1 * L_infill * 30522"""
    return jnp.matmul(one_hot_array, self.word_embeddings.embedding)
  ...
  def __call__(
    ...
    """Customized to accomodate infilling"""
    # Embed
    inputs_embeds = self.word_embeddings(
        input_ids.astype("i4")
    )  ### B * L * 768 for bert base, word embedding: 30522 * 768
    infill_embeds = self.one_hot_embeddings(infill_one_hots)
    inputs_embeds = inputs_embeds.at[:, infill_pos, :].set(infill_embeds)
...
class FlaxBertSelfAttention(nn.Module):
  ...
  def setup(self):
    if self.config.hidden_size % self.config.num_attention_heads != 0:
      raise ValueError(
          "`config.hidden_size`: {self.config.hidden_size} has to be a multiple"
          " of `config.num_attention_heads`                    :"
...
class FlaxBertPreTrainedModel(FlaxPreTrainedModel):
  ...
  def init_weights(
      self, rng: jax.random.PRNGKey, input_shape: Tuple
  ) -> FrozenDict:
    # init input tensors
    input_ids = jnp.zeros(input_shape, dtype="i4")
    infill_one_hots = jnp.zeros((input_shape[0], 1, 30522))
    infill_pos = None
    input_ids = jnp.zeros(input_shape, dtype="i4")
...
class FlaxBertForMaskedLMModule_Infilling(nn.Module):
  _do_init: False
  config: BertConfig
  dtype: jnp.dtype = jnp.float32
...
class FlaxBertForMaskedLM_Infilling(FlaxBertPreTrainedModel):
  module_class = FlaxBertForMaskedLMModule_Infilling
